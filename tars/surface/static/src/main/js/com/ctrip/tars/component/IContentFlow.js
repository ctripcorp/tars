...
ContentFlowGlobal = {
  Flows: [],
  AddOns: {},
  scriptName: 'com.ctrip.tars.component.IContentFlow',
  scriptElement: null,
  Browser: (function() {
    var Opera = window.opera ? true : false,
      IE = document.all && !Opera ? true : false,
      IE6 = IE && typeof(window.XMLHttpRequest) == "undefined" ? true : false,
      IE8 = IE && typeof(document.querySelectorAll) != "undefined" ? true : false,
      IE7 = IE && !IE6 && !IE8 ? true : false,
      ...
      Safari = /Safari/i.test(navigator.userAgent) && !Chrome ? true : false,
      Konqueror = navigator.vendor == "KDE" ? true : false,
      Konqueror4 = Konqueror && /native code/.test(document.getElementsByClassName) ? true : false,
      Gecko = !WebKit && navigator.product == "Gecko" ? true : false,
      Gecko19 = Gecko && Array.reduce ? true : false;
  ...
  addScripts: function(filenames, classloader) {
    for (var i = 0; i < filenames.length; i++) {
      $import(filenames[i], classloader);
    }
  },

  addStylesheet: function(path) {
    $import(path, "CSSClassLoader");
  },

  addStylesheets: function(filenames, classloader) {
    for (var i = 0; i < filenames.length; i++) {
      $import(filenames[i], classloader || "CSSClassLoader");
    }
  },

  initPath: function() {
    /* get / set basic values */
    this.BaseDir = "com.ctrip.tars.component.";
    if (!this.AddOnBaseDir) this.AddOnBaseDir = this.BaseDir;
    if (!this.CSSBaseDir) this.CSSBaseDir = "CSSClassLoader";
  },

  init: function() {
    /* add default stylesheets */
    this.addStylesheet('contentflow'); // FF2: without adding a css-file FF2 hangs on a reload.
    //      I don't have the slidest idea why
    //      Could be timing problem
    this.loadAddOns = [];
    /* add AddOns scripts */
    var AddOns = this.loadAddOns = [];
    for (var i = 0; i < AddOns.length; i++) {
      ...
      //continue;
      //}
      this.addScript(this.AddOnBaseDir + 'ContentFlowAddOn_' + AddOns[i]);
    }
  ...
  onloadInit: function() {
    ...
    for (i = 0; i < ContentFlowGlobal.Flows.length; i++) {
 ...
 * ============================================================
 */
ContentFlowAddOn = function(name, methods, register) {
...
ContentFlowGUIElement = function(CFobj, element) {
 ...
 * ============================================================
 */
ContentFlowItem = function(CFobj, element, index) {
  this.CFobj = CFobj;
  ...
  this.clickItem = function(event) {
    ...
    var index = null; // var index = el.itemIndex ? el.itemIndex : el.parentNode.itemIndex;
    ...
    if (this.Browser.IE) {
      ...
      if (CFobj._reflectionColorRGB) {
        ...
        if (CFobj.conf.reflectionColor == "transparent") {
          RefImg = reflection = this.reflection = document.createElement('img');
      ...
      if (CFobj._reflectionWithinImage) {
        canvas = this.canvas = $CF(document.createElement('canvas'));
      } else {
        canvas = reflection = this.reflection = document.createElement('canvas');
      }
      if (canvas.getContext) {
        ...
        var contextHeight = 0;
        if (CFobj._reflectionWithinImage) {
          contextHeight = height * (1 + CFobj.conf.reflectionGap / 2) * 2;
        } else {
          contextHeight = image.height;
        }
        // -1 for FF 1.5
        contextHeight -= 1;
 ...
 * ============================================================
 */
ContentFlow = function(container, config) {
...
ContentFlow.prototype = {
  ...
  _defaultConf: {
    ...
    calcStepWidth: function(diff) {
      ...
      if (items === 1) {
        stepwidth = diff / this.conf.flowSpeedFactor;
      } else if (Math.abs(diff) > vI) {
        if (diff > 0) {
          stepwidth = diff - vI;
        } else {
          stepwidth = diff + vI;
  ...
  _addItem: function(el, index) {
    ...
    this.conf.visibleItems = Math.min(this.conf.visibleItems, this.items.length - 1, this.conf.maxVisibleItems);
  ...
  _initAddOns: function() {
    ...
    if (this._userConf.useAddOns) {
      if (Object.isString(this._userConf.useAddOns)) {
        loadAddOns = this._userConf.useAddOns.split(" ");
      } else if (Object.isArray(this._userConf.useAddOns)) {
    ...
    for (var i = 0; i < loadAddOns.length; i++) {
      if (loadAddOns[i] == "none") {
        ...
        loadAddOns = [];
        for (AddOn in ContentFlowGlobal.AddOns)
    ...
    for (i = 0; i < loadAddOns.length; i++) {
      AddOn = ContentFlowGlobal.AddOns[loadAddOns[i]];
      if (AddOn) {
    ...
    if (this.conf.flowDragFriction > 0) {
      onDrag = function(event) {
        ...
        if (this.conf.verticalFlow) {
          dist = mouseY - this.Flow.mouseY; // px / or px per sec because _dragFlow wil be called in shorter intervalls if draged fast
          dim = this.Flow.dimensions.height;
        } else {
          dist = mouseX - this.Flow.mouseX; // px / or px per sec because _dragFlow wil be called in shorter intervalls if draged fast
          dim = this.Flow.dimensions.width;
        }
    ...
    if (this.Slider) {
      ...
      this.Slider.setPosition = function(relPos) {
        ...
        if (this.items.length > 1) {
          sPos = (relPos / this.itemsLastIndex) * this.Scrollbar.dimensions.width;
        } else {
          sPos = 0.5 * this.Scrollbar.dimensions.width;
        }
  ...
  _initScrollbarSize: function() {
    ...
    if ((SB = this.Scrollbar) !== null) {
      SB.setDimensions();
      scrollbarHeight = SB.dimensions.height;

      if ((SL = this.Slider) !== null) {
        SL.setDimensions();
        scrollbarHeight += SL.dimensions.height;

        if ((PO = this.Position) !== null) {
  ...
  _initMaxHeight: function() {
    var proportion = 0,
      ...
      Fdim_o = 0;
    if (this.conf.verticalFlow) {
      proportion = screen.width / screen.height;
      Csd = this.Container.style.width;
      Cdim = this.Container.clientWidth;
      Fsd = this.Flow.style.width;
      Fdim = this.Flow.clientWidth;
      Fdim_o = this.Flow.clientHeight;
    } else {
      proportion = screen.height / screen.width;
      Csd = this.Container.style.height;
      Cdim = this.Container.clientHeight;
      Fsd = this.Flow.style.height;
      Fdim = this.Flow.clientHeight;
      Fdim_o = this.Flow.clientWidth;
    }
    ...
    if (this.conf.maxItemHeight <= 0) {
      ...
      var gap = 0;
      if (Csd && Csd != "auto") {
        gap = this.conf.verticalFlow ? 0 : this.conf.reflectionGap;
  ...
  _wheel: function(event) {
    if (!event) event = window.event; // MS
  ...
  _positionItem: function(item, relativeIndex) {
    ...
    /* set z-index */
    els.zIndex = conf.relativeZIndex + Math.round(zIndex * this.items.length); // just for FF
    ...
    if (ContentFlowGlobal.Browser.IE) {
      _setItemSize = function(item, size) {
        ...
        if (!this.conf.fixItemSize) {
          ...
          h = parseInt(item.content.style.height) / 100;
        ...
        if (item.reflection) {
          h = parseInt(item.content.style.height);
  ...
  _setItemOpacity: (function() {
    var _setItemOpacity = null;
    if (ContentFlowGlobal.Browser.IE6) {
      _setItemOpacity = function(item) {
